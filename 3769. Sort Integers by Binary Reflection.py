...
def sort_reversed_bin(x):
    int("".join(reversed(format(4, "b")) ), 2)

def sortByReflection(nums: List[int]) -> List[int]:
    ret_list = sorted(nums, key=lambda x: (int("".join(reversed(format(x, "b")) ), 2), x))
    # print(ret_list)
    return ret_list
...
print(sortByReflection([8,2]))

# print( int("".join(reversed(format(4, "b")) ), 2)   )